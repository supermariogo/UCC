...
int CCodeCounter::CountComplexity(filemap* fmap, results* result)
{
	...
	size_t idx;
	unsigned int cnt, ret, cyclomatic_cnt = 0, ignore_cyclomatic_cnt = 0, main_cyclomatic_distinct_cond_cnt = 1, main_cyclomatic_cnt = 0, function_count = 0, cyclomatic_logic_cnt = 0, main_cyclomatic_logic_cnt = 1, cyclomatic_case_cnt = 0, main_cyclomatic_case_cnt = 1, cyclomatic_default_cnt = 0, cyclomatic_switch_cnt = 0;
	unsigned int cyclomatic_repeated_cond_cnt = 0, main_cyclomatic_repeated_cond_cnt = 0;
	string line, lastline, file_ext, function_name = "";
	string exclude = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_$";
	filemap function_stack;

	stack<unsigned int> cyclomatic_repeated_cond_stack;
	...
	for (fit = fmap->begin(); fit != fmap->end(); fit++)
	{
		...
		if (process_cyclomatic_complexity)
		{
		    ...
		    //std::cout << "ret " << ret << std::endl << line << std::endl;
            //std::cout << "cyclomatic_stack.size() " << cyclomatic_stack.size() << std::endl;
			...
			else if (ret == 2)
			{
				...
				main_cyclomatic_distinct_cond_cnt += cyclomatic_distinct_cond_set.size() - ignore_cyclomatic_cnt;
...
*/
	...
	if (main_cyclomatic_cnt > 0)
	{
		...
		lineElement cc4_element(main_cyclomatic_distinct_cond_cnt, "main");
        function_map[0] = element;
		logical_map[0] = n_element;
		case_map[0] = c_element;
        cond_CC4_map[0] = cc4_element;
	}
	else
	{
