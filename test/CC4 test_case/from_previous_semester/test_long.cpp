...
void CCodeCounter::InitializeCounts()
{
	...
	/*Vikas*/
	cmplx_cyclomatic_count.assign(cmplx_cyclomatic_list.size(), make_pair(0,0));
}
...
int CCodeCounter::CountComplexity(filemap* fmap, results* result)
{
	...
	unsigned int cnt;
	string line;
	unsigned int cyclomatic_count =0;
	...
	for (fit = fmap->begin(); fit != fmap->end(); fit++)
	{
		...
		calculate_cyclomatic_complexity(line, cmplx_cyclomatic_list1, cmplx_cyclomatic_list, bool_cal_list, cyclomatic_count, 1, exclude,
					   "", "", cmplx_cyclomatic_value,casesensitive);
		
		parseFunctionName(line,functionStack ,lastline,cyclomatic_count);
	}
	return 1;
}


int CCodeCounter::calculate_cyclomatic_complexity(const string &base, StringVector &container2,StringVector &container, StringVector &container1, unsigned int &count, int mode, const string &exclude,
					   const string &include1, const string &include2, map<string,int> &counter_container,bool casesensitive)
{


	string::size_type idx,idx1;
	string base1;
	StringVector::iterator vit,vit1;
	...
	if (mode == 1)
	{
		...
		for (vit = container.begin(); vit != container.end(); vit++)
		{
			...
			while (idx != string::npos)
			{
				...
				{
					count=count+ counter_container.find(*vit)->second;
					//single_count++;

					if(find(container2.begin(),container2.end(),*vit) != container2.end())
					{
						for (vit1 = container1.begin(); vit1 != container1.end(); vit1++)
						{
							idx1 = base1.find((*vit1));		
							while (idx1 != string::npos)
							{
								if ((exclude.find(base1[idx1+((*vit1).size())]) == string::npos) && (exclude.find(base1[idx1-1]) == string::npos))
								{
									count=count+1;									
									idx1 = base1.find((*vit1), idx1 + ((*vit1).size() +1));
								}
							}	
						}
					}
...
int CCodeCounter::parseFunctionName(string line, stack<string> &functionStack ,string &lastline,unsigned int &cyclomatic_count)
{
	/* Code to find the function name */
		...
		if(found!=string::npos)
		{
			...
			if (found==1 && lastline.find("class ")==string::npos)
			{
				functionStack.push(lastline);	
			}
			else{
				if(line.find("class ")==string::npos){
				string temp_String = line.substr(0,found);
				lastline= lastline+temp_String;
				functionStack.push(lastline);
				}
			}
		}
		...
		found=line.find('}');
		if(found!=string::npos && !functionStack.empty()){
			string temp_String = functionStack.top();
			functionStack.pop();
			if(functionStack.empty())
			{
				printinCSV(temp_String,cyclomatic_count+1);
				//function_list.push_back(temp_String);
				cout<<"Function Name : " << temp_String<< " Cyclomatic Complexity : " <<cyclomatic_count+1<<endl<<endl;
				cyclomatic_count=0;
				lastline.erase();
				//int i;
				//cin>>i;
...
void CCodeCounter::printinCSV(string funcName,int count)
{
//check if file exists. or may be have a boolean to check if coming for first time
//if coming for first time, create a file with unique name else just open that file in append mode.
//if exists open in append mode.
//write in csv format
//close file.
ofstream output_csv;
std::replace(funcName.begin(), funcName.end(), ',', '|');
output_csv.open(fileName,ios::app);
size_t pos = funcName.find('(');
string str1 = funcName.substr(0,pos);
output_csv << str1 << ",, "<< count << ",, ";
if(count < 10)
	output_csv << "Low" << endl;
else if(count < 20)
	output_csv << "Medium" << endl;
else if(count < 50)
	output_csv << "High" << endl;
else
	output_csv << "Very High" << endl;
output_csv.close();
}
